...
class TestFromCellGrid:
    def test_shape(self):
        ...
        data = rng.standard_normal((7, 4, 4, 4)) + 1j * rng.standard_normal((7, 4, 4, 4))
        am = ABCDMatrix.from_cell_grid(data)
        assert am.shape == (7, 4, 4)

    def test_returns_abcd_matrix(self):
        rng = np.random.default_rng(51)
        data = rng.standard_normal((5, 3, 4, 4)) + 1j * rng.standard_normal((5, 3, 4, 4))
    ...
    def test_single_cell_equals_direct_construction(self):
        ...
        data = rng.standard_normal((5, 1, 4, 4)) + 1j * rng.standard_normal((5, 1, 4, 4))
        am = ABCDMatrix.from_cell_grid(data)
    ...
    def test_two_cells_equals_matmul(self):
        ...
        data = rng.standard_normal((6, 2, 4, 4)) + 1j * rng.standard_normal((6, 2, 4, 4))
        am = ABCDMatrix.from_cell_grid(data)
    ...
    def test_three_cells_equals_sequential_matmul(self):
        ...
        data = rng.standard_normal((4, 3, 6, 6)) + 1j * rng.standard_normal((4, 3, 6, 6))
        am = ABCDMatrix.from_cell_grid(data)
