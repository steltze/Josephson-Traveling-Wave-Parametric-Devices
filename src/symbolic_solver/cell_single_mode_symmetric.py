"""
Symbolic transfer-matrix builder for TWPA/TWPC unit cells for time-periodic
impedance/admittance networks, using a symmetric Pi (shunt-series-shunt) cell.
The transfer matrix T maps the state vector [V[k,n], I[k,n]] -> [V[k,n+1], I[k,n+1]]
for each sideband k simultaneously.
"""
...
class CellSingleModeSymmetric:
    ...
    def _build_Yg_shunt_shifted(
        ...
        #     admittance += Ym_p(self.xi) * self._fourier_basis(mi) * exp(
        #         -I * mi * self.theta
        #     ) + Ym_m(self.xi) * self._fourier_basis(-mi) * exp(
        #         I * mi * self.theta
        #     )
        return admittance
    ...
    def _extract_harmonic_results(
        self, M: int, Zs_t: Expr, Yg_t: Expr, Yg_t_xi: Expr
    ) -> dict[int, tuple[Expr, Expr]]:
        """
        Expand the cell update equations and extract per-harmonic coefficients.

        The cell is a symmetric Pi network: a shunt Yg(t)/2, then a series
        Zs(t), then a second shunt Yg(t)/2. Splitting the shunt admittance
        into two equal halves around the series element (rather than lumping
        it all on one side, as in the plain L cell) makes the transfer matrix
        symmetric (T[0,0] == T[1,1]), matching the reciprocal, symmetric ABCD
        form of a physical Pi section:

            Ia          = I[k, n] - (Yg(t)/2) * V[k, n]
            V[k, n+1]   = V[k, n] - Zs(t) * Ia
            I[k, n+1]   = Ia - (Yg(t)/2) * V[k, n+1]
        ...
        # I[n+1] has a Yg^2*Zs cross-term (two shunt halves + one series pass),
        # one Fourier order higher than the L cell's Yg*Zs term, so harmonics
        # extend to +-3M rather than +-2M.
        ms_nonzero = [m for m in range(-3 * M, 3 * M + 1) if m != 0]
        results = {}
    ...
    def _build_transfer_matrix(
        ...
        Parameters
        ----------
        M        : truncation order (determines harmonic range ±3M)
    ...
    def build_symbolic_transfer_matrix(
        ...
        Yg_t_xi = self._build_Yg_shunt_shifted(Yg_m_p, Yg_m_m)
        results = self._extract_harmonic_results(M, Zs_t, Yg_t, Yg_t_xi)
    ...
    def build_numeric_matrix(
        ...
        k_range = range(k_val - (N_max + 3 * M), k_val + (N_max + 3 * M) + 1)
    ...
    def build_cell_freq_matrices(
        ...
        k_range = range(-(N_max + 3 * M), N_max + 3 * M + 1)
