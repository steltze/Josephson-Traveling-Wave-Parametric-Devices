...
def build_Zs_series(Zs_m):
    ...
    for mi, Zm in enumerate(Zs_m, start=1):
        impedance += (Zm(xi) * fourier_basis( mi) * exp( I * mi * theta)
                    + Zm(xi) * fourier_basis(-mi) * exp(-I * mi * theta))
    return impedance
...
def build_Yg_shunt(Yg_m):
    ...
    for mi, Ym in enumerate(Yg_m, start=1):
        admittance += (Ym(xi) * fourier_basis( mi) * exp( I * mi * theta)
                     + Ym(xi) * fourier_basis(-mi) * exp(-I * mi * theta))
    return admittance
...
def extract_harmonic_results(M, Zs_t, Yg_t):
    ...
    """
    Vn1_t = V[k, n] - Zs_t * Ic[k, n]
    In1_t = Ic[k, n] - Yg_t * Vn1_t

    Vn1_exp = expand(Vn1_t)
    In1_exp = expand(In1_t)

    ms_nonzero = [m for m in range(-2 * M, 2 * M + 1) if m != 0]
    results = {}

    for m in ms_nonzero:
        cv_raw = Vn1_exp.coeff(fourier_basis(m))
        ci_raw = In1_exp.coeff(fourier_basis(m))
        cv = cv_raw.subs(xi, omega[k]).subs(k, k - m)
        ci = ci_raw.subs(xi, omega[k]).subs(k, k - m)
        results[m] = (cv, ci)

    # DC component: subtract all oscillating parts
    V_osc = Add(*[Vn1_exp.coeff(fourier_basis(m)) * fourier_basis(m)
                  for m in ms_nonzero])
    I_osc = Add(*[In1_exp.coeff(fourier_basis(m)) * fourier_basis(m)
                  for m in ms_nonzero])
    cv0 = expand(Vn1_exp - V_osc).subs(xi, omega[k])
    ci0 = expand(In1_exp - I_osc).subs(xi, omega[k])
    results[0] = (cv0, ci0)
...
def build_transfer_matrix(M, ks_state, results):
    """
    Build the symbolic Floquet transfer matrix T.

    Parameters
    ----------
    M        : Floquet truncation order (determines harmonic range ±2M)
...
def build_numeric_matrix(T_sym, dim, M, ks_state, Zs_m, Yg_m,
                          Zs0_val, Yg0_val, theta_val, omega_p_val,
                          omega_val_fn, Zs_num_fn, Yg_num_fn,
                          k_val=0):
    """
