...
def prepare_immitances(Cs, Ls, Cg, epsilons, thetas) -> list[CellImmitance]:
    ...
    return [
        CellImmitance(
            ...
            Yg0_fn=lambda w, C=Cg[i] / (2.0 if i in (0, len(Ls) - 1) else 1.0): (
                1j * w * C
            ),
            Zs_harm_fn=lambda m, w, L=Ls[i], wj_i=wj[i], eps=epsilons[i], i=i: (
                1j * w * L * eps / ((1 - w**2 / wj_i**2) ** 2)
                if (m == 1 and i != 0)
                else 0.0
            ),
...
def test_dispersion_relations(ks_state: list[int] | None = None):
    ...
    M = 1
    freqs = np.linspace(0.0001, 22e9, 1000)
    w = 2.0 * np.pi * freqs
    Nc = 1
    ...
    _, k_RH_ev = bloch_wavenumbers(T_RH.array)  # (Nf, dim)
    _, k_LH_ev = bloch_wavenumbers(T_LH.array)
    print(k_RH_ev.shape)
    k_RH_ev_fwd = k_RH_ev[:, 1]  # most positive k  → forward RH
    ...
    for ax, title, k_ana, k_ev_fwd, k_ev_bwd, wc, passband, ylim in [
        (
            axes[0],
            f"Right-handed ATL  ({dim}x{dim})",
            k_RH_ana,
            k_RH_ev_fwd,
            k_RH_ev_bwd,
            wc_RH,
